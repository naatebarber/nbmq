...
// v0.1.0
// message_hash; 8 | part_hash; 8 | part_count; 1 | part_index; 1 | message_size; 4 | part_size; 4 | chunk_size; 2 | chunk_offset; 2 | data
...
impl Frame<'_> {
    ...
    pub fn parse<'a>(b_frame: &'a [u8]) -> Result<Frame<'a>, Box<dyn Error>> {
        Ok(Frame {
            ...
            part_count: b_frame[9],
            part_index: b_frame[10],
            ...
            chunk_offset: u16::from_be_bytes((&b_frame[20..22]).try_into()?),
            chunk: &b_frame[22..],
        })
    }
}
